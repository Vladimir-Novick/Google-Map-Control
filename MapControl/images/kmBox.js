...
*/
function BoundsBox(m, bounds, opt_options) {
    this.bounds_ = bounds;
    this.setMap(m);
    this.opts = opt_options || {};
    if (this.opts.fit) m.fitBounds(bounds);
    this.map = m;
};
...
BoundsBox.prototype.get_position = function() {  
    if (!this.div_) return null;
    return this.bounds_.getCenter();
};
/**
...
function KmBox(m, point, opt_options) {
    this.opts = opt_options || {};
    ...
    this.setMap(m);
    if (this.opts.fit) m.fitBounds(this.bounds_);
    this.map = m;
};
