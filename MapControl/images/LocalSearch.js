...
*
*      Travel Booking Engine for Professionals
*  Copyright (c) New Concept Travel Technologies Ltd.( www.newconcept.travel )
*/
function PanelFind(){
};
...
LocalSearch.prototype.Init = function() {
    ...
    google.maps.event.addListenerOnce(gblMap, 'tilesloaded', function () {
        $("#LOADINFO").text('LOADED');
    });
...
LocalResult.prototype.marker = function (result) {
    ...
    var t = this.result_.name + " , " + this.result_.formatted_address;
...
LocalResult.prototype.getMarkerText = function() {
    if (typeof (this.textNode_) == "undefined") {
        this.textNode = this.result_.name + " , " + this.result_.formatted_address;
    }
...
LocalResult.prototype.CreateHtml = function() {
    ...
    if (me.result_.formatted_address.indexOf(me.result_.name) >= 0) {
        link_txt =  me.result_.formatted_address;
    } else {
        link_txt = me.result_.name + " <br /> " + me.result_.formatted_address;
    }

    var link = document.createElement('a');
    link.href = "#"; 
    link.onclick = new Function("viewMarkerLink('" + this.id + "')");
    link.appendChild(this.createHTMLNode(link_txt));
